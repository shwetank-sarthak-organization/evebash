...
export default function EventHome({ event, subEvents, basePath }: EventHomeProps) {
    // Format title: "Samarth & Jyoti" -> ["Samarth", "Jyoti"]
    // If no "&", just use title as is.
    const titleParts = event.title.includes("&")
        ? event.title.split("&").map(s => s.trim())
        : [event.title];
    ...
    return (
        <div className="flex flex-col min-h-screen font-sans bg-royal-cream text-royal-maroon disable-scroll-x">
            ...
            <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center space-y-4 px-4 mt-20 animate-fade-in-up max-w-4xl mx-auto">
                    ...
                    <h1 className="text-5xl md:text-8xl lg:text-9xl font-serif text-white drop-shadow-2xl tracking-wide">
                        {titleParts.length === 2 ? (
                            <>
                                {titleParts[0]} <span className="text-royal-gold mx-2 text-6xl md:text-8xl">&</span> {titleParts[1]}
                            </>
                        ) : (
                            event.title
                        )}
                    ...
                    <p className="text-lg md:text-2xl text-royal-cream/90 font-light tracking-widest italic mt-6">
                        {event.date} &bull; {event.description || "Celebrating Love"}
                    </p>
            ...
            {/* Events Grid Section */}
            <section id="events" className="relative py-24 px-4 md:px-12 max-w-7xl mx-auto w-full bg-[#FDFBF7]">
                {/* Note: Removed external pattern URL to avoid mixed content or 404s, can restore if needed */}
                <div className="text-center mb-20 space-y-4">
                    ...
                    <p className="text-royal-maroon/80 italic text-lg max-w-2xl mx-auto font-light">
                        &quot;Join us as we embark on this beautiful journey&quot;
                    </p>
                </div>
